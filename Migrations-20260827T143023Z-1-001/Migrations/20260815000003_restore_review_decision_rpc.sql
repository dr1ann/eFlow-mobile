-- Restore the review-decision RPC used by the Head / Assistant Head review
-- inbox.  Some deployments received the task-submission tables but missed the
-- original review-hardening migration, which leaves submissions visible but
-- makes approval fail with a PostgREST 404.
--
-- The signature deliberately remains compatible with the browser client. The
-- audit hash parameter is retained for old callers but is generated on the
-- server so a browser can never supply the approval audit value.

create extension if not exists pgcrypto;

drop function if exists public.decide_task_review(uuid, boolean, text, text);

create function public.decide_task_review(
  p_task_id uuid,
  p_approve boolean,
...
begin
  ...
  -- reviewer_id is kept authoritative by the leadership-routing trigger:
  -- Head work goes to the Assistant Head and Assistant Head work goes to the
  -- Head. Super Admin retains the existing governance override.
  if not (
  ...
  -- The existing status guard permits this one atomic, audited transition only.
  ...
  -- Notify the person whose work was reviewed. A lead may submit work on a
  -- team's behalf, so notify every distinct worker identity involved rather
  -- than only task.assigned_to. The reviewer never receives their own notice.
  for recipient_id in
...
-- PostgREST otherwise may retain the missing RPC in its schema cache for a
-- short time after the migration is applied.
